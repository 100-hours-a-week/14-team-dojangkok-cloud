# ==============================================
# PROD 환경 — Secondary CIDR 2 (10.2.x.x)
# ALB + ASG(FE/BE), DB 단일 인스턴스, Blue/Green
# Bastion 제거 → SSM으로 Private 인스턴스 접근
# ==============================================

locals {
  secondary_cidr = "10.2.0.0/18"
  public_cidr    = "10.2.0.0/24"   # AZ-a, NAT + 실 리소스
  alb_dummy_cidr = "10.2.6.0/28"   # AZ-c, ALB 2-AZ 요건 충족용 (리소스 배치 X)
  fe_cidr        = "10.2.1.0/24"
  be_cidr        = "10.2.2.0/24"
  rdb_cidr       = "10.2.3.0/24"
  mq_cidr        = "10.2.4.0/24"
  cache_cidr     = "10.2.5.0/24"
...
module "networking" {
  ...
  public_subnets = {
    "public"   = { cidr = local.public_cidr, az = "a" }
    "public-c" = { cidr = local.alb_dummy_cidr, az = "c" } # ALB 2-AZ 요건 충족용
  }

  private_subnets = {
    "pri-fe"    = { cidr = local.fe_cidr, az = "a" }
    "pri-be"    = { cidr = local.be_cidr, az = "a" }
    "pri-rdb"   = { cidr = local.rdb_cidr, az = "a" }
    "pri-mq"    = { cidr = local.mq_cidr, az = "a" }
    "pri-cache" = { cidr = local.cache_cidr, az = "a" }
  }
...
module "asg_fe" {
  ...
  subnet_ids           = [module.networking.private_subnet_ids["pri-fe"]]
...
module "asg_be" {
  ...
  subnet_ids           = [module.networking.private_subnet_ids["pri-be"]]
...
module "db_servers" {
  ...
  instances = {
    mysql = {
      ...
      subnet_id            = module.networking.private_subnet_ids["pri-rdb"]
    ...
    rabbitmq = {
      ...
      ami                  = var.docker_ami_id
      subnet_id            = module.networking.private_subnet_ids["pri-mq"]
    ...
    redis = {
      ...
      ami                  = var.docker_ami_id
      subnet_id            = module.networking.private_subnet_ids["pri-cache"]
