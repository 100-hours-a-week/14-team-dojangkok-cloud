# 5단계: V3 모니터링 설계 (v1.1.0)

- 작성일: 2026-03-09
- 최종수정일: 2026-03-11
- 작성자: infra (claude-code)
- 상태: draft
- 관련문서: [design-step5.md](../design-step5.md) (v3.0.0), [iac-plan.md](./iac-plan.md) (v1.0.0), [cicd-plan.md](./cicd-plan.md) (v1.0.0)

> **스코프**: V2 모니터링 스택(Prometheus/Loki/Grafana/Tempo + Alloy)을 V3 K8S 환경에 맞게 확장.
> **하이브리드 모니터링**: 수집은 K8S 클러스터 안, 저장·시각화는 외부 EC2.

---

## 목차

1. [모니터링 배치 전략 — 하이브리드](#1-모니터링-배치-전략--하이브리드)
2. [V2 → V3 변경 요약](#2-v2--v3-변경-요약)
3. [아키텍처](#3-아키텍처)
4. [메트릭 수집 — Alloy DaemonSet](#4-메트릭-수집--alloy-daemonset)
5. [K8S 메트릭 — kube-state-metrics](#5-k8s-메트릭--kube-state-metrics)
6. [애플리케이션 메트릭](#6-애플리케이션-메트릭)
7. [로그 수집](#7-로그-수집)
8. [Grafana 대시보드](#8-grafana-대시보드)
9. [알림](#9-알림)
10. [구현 순서](#10-구현-순서)
...
K8S 클러스터에 모니터링을 도입할 때 배치 위치에 대해 세 가지 접근이 있다.

| 방식 | 구성 | 장점 | 단점 |
|------|------|------|------|
| **클러스터 내부** | Prometheus/Grafana/Loki 전부 K8S Pod | K8S 서비스 디스커버리 활용, 설치 간편 | **클러스터 장애 시 모니터링도 사망** |
| **클러스터 외부** | 전부 외부 VM | 클러스터와 독립적 생존 | 메트릭 수집 경로 복잡 (remote scrape) |
| **하이브리드** | 수집기만 K8S 안, 저장·시각화는 외부 | 클러스터 장애에도 데이터 유지 + 수집 편의 | push 기반 설정 필요 |

### 결정: 하이브리드

| 위치 | 컴포넌트 | 역할 |
|------|---------|------|
| **K8S 클러스터 안** | Alloy DaemonSet, kube-state-metrics | 메트릭·로그 수집 → 외부로 push |
| **외부 EC2 (기존 유지)** | Prometheus, Loki, Tempo, Grafana | 저장·시각화·알림 |

**선택 근거**:
- **장애 독립성**: 클러스터가 죽어도 기존 메트릭·로그 데이터와 대시보드가 살아있어 장애 원인 분석 가능
- **V2 자산 재활용**: 기존 모니터링 EC2(dev/prod)와 대시보드를 그대로 사용. 추가 인프라 비용 없음
- **K8S 리소스 절약**: Prometheus(200m/512MB) + Grafana(100m/256MB)를 클러스터에 배치하지 않아 Worker 여유 확보
- **수집 편의**: Alloy DaemonSet이 K8S 서비스 디스커버리로 Pod 메트릭/로그를 자동 수집 후 remote-write/push
...
5. **N-1 여유** — (Allocatable - Used) / Allocatable, 43% 기준선 표시
...
| v1.1.0 | 2026-03-11 | 하이브리드 배치 전략 섹션 추가(§1), 선택 근거 명시. 구 수치 수정(N-1 69%→43%). 섹션 번호 재배치. |
