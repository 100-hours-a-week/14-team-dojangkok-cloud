...
11. [데이터베이스 배치 전략](#11-데이터베이스-배치-전략)
...
#### 1. VM 기반 장애 복구 지연이 야기하는 비즈니스 리스크 및 신뢰성 훼손
- 현재 FE/BE 등 일부 서비스는 ASG/MIG를 통해 인스턴스 단위 복구를 지원하지만, 인스턴스 장애 감지 후 새 VM을 프로비저닝하여 서비스에 투입하기까지 **수 분의 공백**이 발생한다.
- 도장콕의 핵심 가치는 부동산 계약 현장에서 **사용자의 안전한 의사결정을 실시간으로 지원**하는 것이다. 특히 부동산 거래가 활발한 **주간(09~21시)에 수 분간의 애플리케이션 서비스 장애가 발생할 경우**, 사용자가 AI의 검토 없이 치명적인 계약 리스크를 떠안게 되는 **금전적·정신적 피해로 직결**될 수 있다.
- K8S의 자동 복구(Self-Healing) 체계는 클러스터 내 워크로드(App)의 상태를 중앙에서 주기적으로 감시한다. 노드나 파드 장애 시 이미 확보된 잉여 워커 노드 자원에 즉각적으로 파드를 재스케줄링하여 **복구 시간을 수초 내외로 단축**함으로써, 우리 서비스의 높은 가용성과 신뢰도를 보장할 수 있다. (단, DB 등 상태 저장 워크로드는 별도의 EC2 HA 구성을 통해 K8S 외부에서 안정성을 확보한다.)
...
- **대응 방안**: 초기에는 K8S가 AWS 자원을 직접 제어하게 만드는 복잡한 연동을 배제하고, 역할을 명확히 분리하는 직관적인 방식으로 우회한다.
  - **트래픽 인입 (ALB 정적 매핑 + 차세대 Gateway API)**: AWS 환경에서 표준으로 쓰이는 AWS Load Balancer Controller (ALBC)를 설치하면 K8S Ingress/Gateway 리소스 선언 시 외부 ALB가 동적으로 자동 생성된다. 하지만 우리는 인프라 생명주기(Terraform)와 K8S 리소스(ArgoCD)를 철저히 분리하기 위해, 인프라(ALB/NLB)는 기존처럼 Terraform으로 고정 생성한다. K8S 내부에는 통합 Gateway API 컨트롤러인 NGINX Gateway Fabric을 띄우고 이를 NodePort로 노출시킨 뒤, 외부의 AWS 타겟 그룹이 이 EC2 고정 노드포트를 바라보게 하는 **정적 매핑 방식**을 채택한다(Part 4에서 상세 서술).
...
> DB, Redis, RabbitMQ 등 상태 저장(Stateful) 워크로드는 외부 EC2에서 영구 운영되므로 K8S 리소스 산정에서 제외된다. BE Request 700m: 4대 평균 피크 306m × 2.3x headroom. 상세: [node-sizing.md](./node-sizing.md)

### 워크로드 합산 (DB 제외, Stateless App 전용)
...
트래픽 인입·배포·관측성을 위해 클러스터 전역에 배포되는 공유 컴포넌트(Gateway Fabric, CD, 모니터링 등)를 포함한 합산이다. (Stateful 워크로드 EC2 잔류 확정)

**시나리오 A — Prod 기본 HA (채택)**

| 카테고리 | 구성 | CPU Request | RAM Request |
|----------|------|-------------|-------------|
| **App** | FE×2, BE×2, AI×1 | 2200m | 4.75GB |
| 공유 컴포넌트 | Gateway Fabric, ArgoCD, Alloy, kube-state-metrics | ~690m | ~1.1GB |
...
|----------|---------|---------|----------|
| 기본 HA (FE×2, BE×2) | — | — | ~2890m |
| FE +1 (→3대) | +250m | +512MB | ~3140m |
| BE +1 (→3대) | +700m | +1.5GB | ~3590m |
| FE+BE 각 +1 (Max) | +950m | +2.0GB | ~3840m |
...
섹션 3에서 산출한 Prod HA 워크로드(~2890m)와 HPA 스케일아웃 시(최대 ~3840m)를 수용할 클러스터 노드를 선정한다. k6 부하테스트에서 BE 피크 908m(단일 인스턴스)이 확인되었고, T시리즈 CPU 크레딧 고갈이 실측으로 확인되었으므로(BE 90.3%, FE 92.8%), **N-1 장애 내성 + HPA 확장 여유** 를 확보하는 것이 핵심이다.
...
### Worker — t4g.large 확장형 3-AZ (workers_per_az)

DB 워크로드가 K8S 이관 계획에서 제외되고, 모니터링 서버(Prometheus/Grafana)도 외부 EC2에서 유지됨에 따라 Prod 워크로드(~2890m)가 크게 감소했다. 부하테스트 피크와 HPA 팽창(~3840m)을 고려하여 3-AZ 균등 배치를 설계한다.

| 옵션 (workers_per_az) | AZ 배치 | Alloc CPU | Alloc RAM | 월비용 | HA 활용률 | N-1 (HA) |
|------|---------|-----------|-----------|--------|----------|----------|
| t4g.large × 3 (v=1) | 1-1-1 | 4.5v | 19.5GB | $147 | 64% | **96% (위험)** |
| **t4g.large × 6 (v=2)** | **2-2-2** | **9.0v** | **39.0GB** | **$294** | **32%** | **39% (매우 여유)** |

- **HA 활용률** = Prod 필요 CPU(~2890m) ÷ Alloc CPU
- **N-1 (HA)** = Prod 필요 CPU ÷ (Alloc CPU − 노드 1대분)

DB 워크로드 이탈과 모니터링 서버 외부 배치로 인해 **t4g.large × 6** 구성은 N-1 상황에서도 39%의 점유율만 보이며, HPA 팽창(최대 3840m) 시에도 51% 점유로 매우 안정적이고 강력한 인프라가 된다. 
초기 인프라 셋업 단계에서는 비용 절감을 위해 `workers_per_az = 1` (1-1-1, 총 3대) 배포를 진행하고, Prod 트래픽 전환 시점에 `workers_per_az = 2` (2-2-2, 총 6대) 로 파드(Pod) 무중단 스케일아웃 및 anti-affinity를 보장하는 전략을 취한다.

> **결정: Worker t4g.large (2vCPU, 8GB) — 트래픽 규모에 따라 3대(1-1-1) 또는 6대(2-2-2)로 확장 가능한 Terraform `workers_per_az` 변수화 연동.**
...
**2-2-2 균등 배치** (AZ 2a에 CP+W1+W2, AZ 2b에 W3+W4, AZ 2c에 W5+W6)가 3개 AZ 모두에서 Worker 2대씩을 확보하여, 어느 AZ에서든 노드 1대 장애 시 같은 AZ의 나머지 노드로 재스케줄링할 수 있다. 단일 AZ 장애(2대 손실) 시에도 나머지 4대(6.0v)로 Prod HA 워크로드(~2890m)를 48% 활용률로 여유롭게 수용 가능하다. FE/BE는 Anti-Affinity로 AZ에 분산하여 어느 AZ가 다운되어도 서비스를 유지한다.
...
> **결정: 3-AZ 2-2-2 균등 배치 — AZ 2a(CP+W1+W2), AZ 2b(W3+W4), AZ 2c(W5+W6). N-1 39%, AZ 장애 48%**
...
| **Calico** | CNI + NetworkPolicy 사실상의 표준, 거대한 커뮤니티 레퍼런스. 향후 eBPF 데이터플레인 적용을 통한 성능 확장 가능 | 기본적으로 iptables 기반이므로 트래픽 라우팅 체인이 다소 복잡함 |
| **Cilium** | 압도적 성능(태생적 eBPF), L7 보안 및 강력한 트래픽 가시성(Hubble) | 높은 설정 복잡도, 스타트업 초기 인프라에는 다소 오버스펙 |

V2의 보안그룹 격리를 K8S NetworkPolicy로 계승하는 것이 핵심 요구이다. eBPF의 우수한 관측성과 네트워크 성능 이점을 제공하는 Cilium도 매력적인 후보다. 하지만 우리 팀의 현재 운영 리소스 하에서는 **검증된 안정성과 문제 해결(Troubleshooting) 레퍼런스의 풍부함**을 최우선으로 하여 **Calico**를 채택한다. 향후 고성능 트래픽 처리가 필요해질 경우, Calico의 eBPF 데이터플레인 모드를 활성화하여 Cilium에 준하는 성능을 끌어올릴 수 있는 아키텍처적 유연성도 담보되어 있다.
...
> **결정: Calico + VXLAN — CNI+NetworkPolicy 단일 컴포넌트, 모든 Pod 트래픽 VXLAN 캡슐화. AWS source/dest check 비활성화, veth_mtu=8951 튜닝 적용**
...
> **결정: iptables (Kubeadm 기본값 유지) — 현재 규모에서 성능 오버헤드 미미, 운영 단순화**
...
K8S 내부에는 영구 저장이 필요한 인프라 Pod(ArgoCD 등)가 존재하므로, 이들을 위한 영구 스토리지(EBS) 전략이 필요하다. 모니터링 데이터(Prometheus/Loki)는 외부 EC2에서 저장하므로 K8S PVC 대상이 아니다.
...
| 정책 | 대상 | 동작 | 이유 |
|------|------|------|------|
| **Delete** | 임시 캐시, 재생성 가능한 데이터 | PVC 삭제 시 EBS도 삭제 | 비용 절약 |
...
> **결정: AWS EBS CSI Driver + gp3 Default StorageClass (WaitForFirstConsumer). Reclaim Policy는 데이터 유지 필요성에 따라 Delete/Retain 분리.**
...
EC2 인프라 레벨에서 이를 **3-AZ(2a/2b/2c) 분산 클러스터**로 구성하여 자동 Failover를 확보한다(§12 참조).

### EC2 영구 배치 

아키텍처의 복잡도를 낮추고 데이터 안정성을 극대화하기 위해 **Stateful 워크로드는 K8S 밖의 영구 EC2 클러스터에서 완전히 분리하여 운영**한다.

- **장애 격리**: K8S 컨트롤 플레인 장애가 DB 가용성에 영향을 주지 않음.
- **EBS AZ 종속 회피(§10)**: K8S에서 영구 스토리지가 특정 AZ에 묶이는 단점을 피하고 DB 레벨 자체 HA에 집중함.
- **리소스 완전 분리**: DB 전용 EC2 인스턴스 사용으로 CPU/메모리 경합 원천 차단.

```
[K8S Cluster — K8S Private 서브넷]
  FE, BE, AI-Server (Deployment)
  Alloy, kube-state-metrics, ArgoCD (PVC: ArgoCD만)
  Gateway Fabric (DaemonSet)
       │
       │ ExternalName Service (투명성 보장)
       ▼
[EC2 — 별도 Data 서브넷, 3-AZ 분산]
  MySQL, MongoDB, Redis, RabbitMQ, ChromaDB (§12 DB HA 클러스터)
```

K8S Pod가 EC2 DB에 접근할 때는 `dojangkok` Namespace에 **ExternalName Service**를 생성하여 도메인 기반으로 라우팅한다. 이를 통해 K8S 안의 백엔드 앱은 DB가 K8S 안에 있는지 밖에 있는지 알 필요 없이 K8S Service명으로 투명하게 접근할 수 있다.

> **결정: Stateful(MySQL/MongoDB/Redis/RabbitMQ/ChromaDB) 워크로드는 같은 VPC 내의 격리된 EC2 클러스터에서 영구 운영한다.**
...
이를 위해 기존의 단일 인스턴스 수준을 넘어, 주요 인프라 구성요소 전반을 고가용성 EC2 클러스터로 개편한다. (K8S 내부로 이관하지 않고 외부 클러스터로 영구 운영)
...
> **결정: MySQL(InnoDB Cluster), MongoDB(ReplicaSet), Redis(Sentinel), RabbitMQ(Quorum Queue) 모두 3-AZ(2a, 2b, 2c) 풀 클러스터 분산 배치로 운영하여 완전 자동 Failover를 실현한다. 단, ChromaDB는 단일 노드 운영 및 스냅샷 복구 전략을 채택한다.**
...
> **결정: NGINX Gateway Fabric — 기능적으로 Ingress와 차이 없으나, 동결된 Ingress 스펙 대신 활발한 Gateway API 표준 + 검증된 NGINX 엔진 활용**
...
> **결정: Gateway Fabric NodePort 전용 노출. DaemonSet + ALB health check로 SPOF 제거. 보안 은폐(ClusterIP) 및 라우팅 룰 GitOps 관리를 위해 모든 외부 트래픽은 Gateway Fabric을 통해서만 진입**
...
> **결정: ALB에서 TLS 종료 — 내부 HTTP. ACM 인증서 활용**
...
| Role / RoleBinding | NS 단위 권한. `monitoring` NS의 Role은 `dojangkok` NS 리소스에 접근 불가 |
| ServiceAccount | NS 소속. `dojangkok` NS의 `be-sa`와 `monitoring` NS의 `prometheus-sa`는 완전 별개 |
| Secret / ConfigMap | NS 격리. `dojangkok` NS의 DB 패스워드 Secret을 `monitoring` NS Pod에서 직접 참조 불가 |
...
| NS | 워크로드 | 비고 |
|----|---------|------|
| `dojangkok` | FE, BE, AI Server | 비즈니스 워크로드 |
| `monitoring` | Alloy (DaemonSet), kube-state-metrics | 수집기만 배치 (Prometheus/Grafana는 외부 EC2) |
| `argocd` | ArgoCD | Helm 차트 기본 NS |
| `nginx-gateway` | NGINX Gateway Fabric | Helm 차트 기본 NS |
| `kube-system` | CoreDNS, kube-proxy, Calico, EBS CSI Driver | kubeadm 기본 |
...
| dojangkok | 외부(VPC) | BE → EC2 DB (MySQL 등) | `ExternalName` Service (`mysql.dojangkok.svc.cluster.local` → EC2 Private IP) |
| dojangkok | 외부(인터넷) | AI Server → RunPod | NAT Instance 경유 |
| monitoring | 전체 NS | Alloy 메트릭·로그 수집 | K8S 서비스 디스커버리로 대상 자동 탐지 |
| argocd | 전체 NS | ArgoCD 배포 관리 | K8S API 경유 |

### 외부 DB 연동 패턴 (ExternalName Service)

K8S 클러스터 바깥의 EC2 DB에 접근하기 위해 `dojangkok` 네임스페이스 내에 **ExternalName (또는 Endpoints) Service**를 생성하여 연결한다. 
이렇게 하면 Application(백엔드) 입장에서는 투명성이 보장되며, 향후 DB IP가 변경되더라도 코드 수정 없이 Service YAML만 업데이트하여 유연하게 대처할 수 있다.

> **결정: 역할별 5개 NS — dojangkok(비즈니스 앱 + ExternalName DB 서비스) / monitoring / argocd / nginx-gateway / kube-system.**
...
| 4 | Alloy → 전체 Pod (메트릭 포트) | Ingress | 전체 | 모니터링 블랙아웃 |
...
Alloy       → 전체 Pod (메트릭 스크래핑 + 로그/트레이스 수집)
```
...
| Ingress | 공통 필수 #1, #4 (kubelet, Alloy) | |
| Egress | BE → 외부 EC2 DB (MySQL 등) | VPC Private IP 대역 송신 허용 |
| Egress | AI Server → 외부 인터넷 (RunPod) | 공통 필수 #5 |
| Egress | 전체 → kube-dns | 공통 필수 #2 |

**`monitoring` NS** (Alloy, kube-state-metrics):

| 방향 | 룰 | 비고 |
|------|-----|------|
| Egress | Alloy → **전체 NS** Pod | Cross-NS 메트릭 스크래핑, `namespaceSelector: {}` (all) |
| Egress | Alloy → **외부 EC2** (모니터링 서버) | remote-write (Prometheus), push (Loki) |
...
> **결정: Default Deny + Whitelist — NS별 개별 적용, Cross-NS는 namespaceSelector 명시 필수. 기본 차단 + 허용 방식**
...
- K8S API 접근이 필요한 워크로드(ArgoCD, EBS CSI Driver, Alloy 등)에만 전용 SA + 최소 Role을 부여한다
...
|----|---------|----------|-------------|------|
| `dojangkok` | `fe-sa`, `be-sa`, `ai-sa` | **빈 Role & 미마운트** | 철저히 차단됨 | 비즈니스 로직만 수행. `automountServiceAccountToken: false`를 지정하여 해킹 시에도 K8S 조작 원천 차단 |
| `monitoring` | `alloy-sa` | **ClusterRole** | 전체 NS 읽기 | K8S 서비스 디스커버리로 모든 NS의 Pod/Service 메트릭·로그 수집 |
...
`dojangkok` NS는 자기 NS 안에서만 동작하므로 별도의 ClusterRole 권한 부여를 지양한다.
`monitoring`, `argocd`, `nginx-gateway`, `kube-system`은 다른 NS 리소스를 컨트롤해야 하므로 ClusterRole이 필수다.
...
2. **automountServiceAccountToken: false 기본** — K8S API 접근이 전혀 필요 없는 App 워크로드(FE, BE, AI 파드)는 이 속성을 `false`로 주입하여, 애플리케이션 코드가 해킹되어도 컨테이너 내부에서 K8S API(apiserver)를 악의적으로 조작할 수 없도록 권한을 원천 차단한다.
...
> **결정: 워크로드별 전용 SA + 최소 권한 RBAC — NS별 Role/ClusterRole 분리 적용. 구체 SA/Role 목록은 구현 단계에서 정의**
...
| Q4 | 노드 사이징 | t4g.large × 6 + CP t4g.medium × 1 | 부하테스트 반영, N-1 39%, 3-AZ 2-2-2 |
| Q6 | AZ 배치 | 2a(CP+W1+W2) / 2b(W3+W4) / 2c(W5+W6) | 3-AZ 균등, AZ 장애 48% 수용 |
...
| Q17 | Namespace | 역할별 5개 NS (dojangkok/monitoring/argocd/nginx-gateway/kube-system) | 보안 정책 범위 분리, Helm 기본값 존중 |
| Q2 | Stateful 배치 | K8S 외부 EC2 영구 운영 | K8S 종속성(AZ 등) 탈피, 인프라 추상화(ExternalName), 가용성(HA) 독립 확보 |
