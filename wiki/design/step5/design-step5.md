...
K8S 내부에는 메트릭/로그 데이터를 영구 저장해야 하는 인프라 Pod(Prometheus, Grafana, ArgoCD 등)가 존재하므로, 이들을 위한 영구 스토리지(EBS) 전략이 필요하다.
...
> **결정: AWS EBS CSI Driver + gp3 Default StorageClass (WaitForFirstConsumer). Reclaim Policy는 데이터 유지 필요성(Prometheus vs ArgoCD)에 따라 Delete/Retain 분리.**
...
### EC2 영구 배치 (Stateful K8S 이관 취소)

초기에는 K8S 운영 경험 축적 후 StatefulSet으로 이관하는 다단계 전략을 고려했으나, 아키텍처의 복잡도를 낮추고 데이터 안정성을 극대화하기 위해 **Stateful 워크로드는 K8S 밖의 영구 EC2 클러스터로 완전히 분리**하기로 최종 결정했다.
...
> **최종 결정: Stateful(MySQL/MongoDB/Redis/RabbitMQ/ChromaDB) 워크로드는 K8S 내부로 이관하지 않고 같은 VPC 내의 격리된 EC2 클러스터에서 영구 운영한다.**
...
DB 파드 이관 계획(Phase 2)을 폐기함에 따라, K8S 클러스터 바깥의 EC2 DB에 접근하기 위해 `dojangkok` 네임스페이스 내에 **ExternalName (또는 Endpoints) Service**를 생성하여 연결한다. 
이렇게 하면 Application(백엔드) 입장에서는 DB가 K8S 안에 파드로 떠 있는지 밖의 클라우드 EC2에 떠 있는지 알 필요가 없으며(투명성 보장), 향후 DB IP가 변경되더라도 코드 수정 없이 Service YAML만 업데이트하여 유연하게 대처할 수 있다.
