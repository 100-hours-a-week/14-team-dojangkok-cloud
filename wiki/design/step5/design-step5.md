...
| 항목 | 현행 V2 | K8S 전환 | 차이 |
|------|---------|---------|------|
| AWS (Dev+Prod 개별 인프라) | ~$342 | - | |
| GCP 비GPU (AI Server, ChromaDB 등) | ~$230 | - | |
| K8S AWS (단일 클러스터) | - | ~$456 | |
| **인프라 합계** | **~$572** | **~$456** | **-$116/월 (-20%)** |

> 현행 V2는 Dev/Prod가 완전히 별개의 인프라(AWS EC2 7대 × 2env + GCP VM)로 운영되지만, K8S는 **단일 클러스터에서 Namespace 분리**로 인프라 1세트만 유지합니다. 또한 ASG min=max=1로 실질적 이중화가 없는 현행과 달리, K8S는 워커 노드 여유 자원 내에서 Pod를 추가할 수 있어 **소규모 스케일링 시 추가 비용이 발생하지 않습니다**.
