...
3. [노드 사이징 — 인스턴스 타입, 수량](#3-노드-사이징--인스턴스-타입-수량)
4. [AZ 배치 — 노드별 워크로드 배분](#4-az-배치--노드별-워크로드-배분)
5. [리소스 산정 — Request/Limit 방법론](#5-리소스-산정--requestlimit-방법론)



**Part 2. 데이터 및 특수 워크로드 전략**

6. [DB 배치 — K8S 내부 vs 외부](#6-db-배치--k8s-내부-vs-외부) ⚠ 논의 필요
7. [DB HA 토폴로지](#7-db-ha-토폴로지) ⚠ 논의 필요
8. [GPU 워크로드 — K8S 내부 vs RunPod 외부](#8-gpu-워크로드--k8s-내부-vs-runpod-외부)
...
단일 Control Plane(2a) 장애 시, 클러스터 제어권(스케줄링, 배포)은 일시 상실되나 기존 Worker 노드(2a, 2c)에 배포된 Data Plane(FE, BE 파드 및 라우팅 룰)은 정상 작동하여 **사용자 서비스 중단은 발생하지 않습니다**. CP 복구는 S3에 자동화된 etcd 스냅샷을 통해 수행합니다(§2 단점 1 참조).

> **결정: 2:2 균등 배치 — AZ 2a(CP+W1+W2), AZ 2c(W3+W4). 양쪽 AZ 모두 재스케줄링 가능**

---

## 5. 리소스 산정 — Request/Limit 방법론
...
## 8. GPU 워크로드 — K8S 내부 vs RunPod 외부
...
**NAT Gateway**(월 ~$32 + 데이터)는 AWS 관리형으로 자동 고가용성을 제공하지만 비용이 높습니다. **NAT Instance × 1**(월 ~$3.8)은 저비용이며 V2에서 운영 경험이 있지만, Q4에서 결정한 2:2 AZ 배치(N-1 생존)와 **논리적으로 상충**합니다. NAT Instance가 2a에만 있을 때 2a AZ 장애가 발생하면, 2c에 살아남은 Worker(W3, W4)가 인터넷 출구를 잃어 RunPod API 호출·ECR 이미지 풀·HPA 스케일아웃이 모두 불가능해지므로, 2:2 배치로 확보한 고가용성이 무의미해집니다.
...
> **결정: NAT Instance × 2 (t4g.nano, AZ별 배치) — 라우팅 테이블 분리로 AZ 독립 인터넷 출구 확보. 2:2 배치(N-1 생존) 철학과 일치. 월 +$3.8 추가**
...
Q10에서 NGINX Gateway Fabric을 Ingress로 확정한 이상, **모든 외부 트래픽은 Gateway Fabric을 통해서만 진입**하는 구조가 자연스럽습니다. FE/BE를 별도 NodePort로 직접 노출하면 라우팅 룰이 ALB와 Gateway에 이원화되고, Gateway Fabric의 관측성(access log, 메트릭) 이점이 사라집니다.
...
| Q12 | 외부 통신 | NAT Instance × 2 (t4g.nano, AZ별) | 2:2 AZ 배치(N-1 생존)와 일치, 월 +$3.8 |
