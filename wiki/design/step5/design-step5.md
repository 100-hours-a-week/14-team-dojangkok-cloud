...
3. [설계 질문 목록](#3-설계-질문-목록)
4. [Q1: 클러스터 전략 — 단일 vs 멀티? 어떤 클라우드?](#4-q1-클러스터-전략--단일-vs-멀티-어떤-클라우드)
5. [Q2: DB 위치 — K8S 내부 vs 외부?](#5-q2-db-위치--k8s-내부-vs-외부)
6. [Q3: GPU 처리 — K8S 내부 vs RunPod 외부?](#6-q3-gpu-처리--k8s-내부-vs-runpod-외부)
7. [Q4: 노드 사이징 — 인스턴스 타입, 수량](#7-q4-노드-사이징--인스턴스-타입-수량)
8. [Q5: DB HA 토폴로지](#8-q5-db-ha-토폴로지)
9. [Q6: AZ 배치 — 노드별 워크로드 배분](#9-q6-az-배치--노드별-워크로드-배분)
10. [Q7: 리소스 산정 — Request/Limit 방법론](#10-q7-리소스-산정--requestlimit-방법론)
11. [Q8: CNI 선택](#11-q8-cni-선택)
12. [Q9: 네트워크 대역](#12-q9-네트워크-대역--pod-cidr-service-cidr)
13. [Q10: Ingress](#13-q10-ingress--외부-트래픽-진입)
14. [Q11: Service 노출 전략](#14-q11-service-노출-전략)
15. [Q12: 외부 통신](#15-q12-외부-통신--nat-runpod-호출-경로)
16. [Q13: NetworkPolicy](#16-q13-networkpolicy--pod-간-통신-제어)
17. [Q14: TLS 종료 지점](#17-q14-tls-종료-지점)
18. [설계 결정 요약](#18-설계-결정-요약)
19. [비용 비교](#19-비용-비교)
20. [장애 대응 & Failover](#20-장애-대응--failover)
21. [부록](#21-부록)
...
K8S 전환 시 풀어야 할 핵심 고민을 아래 순서로 해결합니다.

| # | 질문 | 의존 | 해결 |
|---|------|------|------|
| Q1 | 클러스터를 어떻게 구성할까? (단일/멀티, AWS/GCP) | - | [섹션 4](#4-q1-클러스터-전략--단일-vs-멀티-어떤-클라우드) |
| Q2 | DB를 K8S 안에 넣을까 밖에 둘까? | Q1 | [섹션 5](#5-q2-db-위치--k8s-내부-vs-외부) |
| Q3 | GPU(vLLM)를 어떻게 처리할까? | Q1 | [섹션 6](#6-q3-gpu-처리--k8s-내부-vs-runpod-외부) |
| Q4 | Worker 노드 몇 대, 어떤 사양? | Q1~Q3 | [섹션 7](#7-q4-노드-사이징--인스턴스-타입-수량) |
| Q5 | DB HA를 어떤 토폴로지로? | Q2, Q4 | [섹션 8](#8-q5-db-ha-토폴로지) |
| Q6 | AZ를 어떻게 분배할까? | Q4, Q5 | [섹션 9](#9-q6-az-배치--노드별-워크로드-배분) |
| Q7 | Request/Limit을 어떻게 잡을까? | Q4~Q6 | [섹션 10](#10-q7-리소스-산정--requestlimit-방법론) |
| Q8 | CNI를 뭘 쓸까? | Q1 | [섹션 11](#11-q8-cni-선택) |
| Q9 | 네트워크 대역을 어떻게 잡을까? | Q8 | [섹션 12](#12-q9-네트워크-대역--pod-cidr-service-cidr) |
| Q10 | Ingress를 뭘 쓸까? | Q8 | [섹션 13](#13-q10-ingress--외부-트래픽-진입) |
| Q11 | Service 노출 전략은? | Q10 | [섹션 14](#14-q11-service-노출-전략) |
| Q12 | 외부 통신을 어떻게 할까? | Q1, Q3 | [섹션 15](#15-q12-외부-통신--nat-runpod-호출-경로) |
| Q13 | NetworkPolicy를 어떻게 설정할까? | Q8, Q10 | [섹션 16](#16-q13-networkpolicy--pod-간-통신-제어) |
| Q14 | TLS를 어디서 종료할까? | Q10, Q11 | [섹션 17](#17-q14-tls-종료-지점) |

---

## 4. Q1: 클러스터 전략 — 단일 vs 멀티? 어떤 클라우드?

V2 아키텍처는 AWS(FE/BE/DB)와 GCP(AI Server/ChromaDB)로 이원화되어 있습니다. 팀원은 클라우드 2, 풀스택 2(FE/BE), AI 1로 총 5명이며, 섹션 1에서 기술한 것처럼 **상대 클라우드 이해도 격차**로 인한 IaC 오류와 Spot 선점 대응 지연이 반복되고 있었습니다.

검토한 선택지는 세 가지입니다. 첫째, **AWS+GCP 멀티클러스터**로 기존 구조를 유지하면 GCP GPU 크레딧을 계속 활용할 수 있지만, 운영 복잡도가 2배로 유지되고 팀 이해도 격차 문제가 해소되지 않습니다. 둘째, **GCP 단일 클러스터**로 통합하면 GPU 크레딧을 활용할 수 있으나, DB(MySQL)가 이미 AWS에 있어 마이그레이션 비용과 리스크가 발생합니다. 셋째, **AWS 단일 클러스터**로 통합하면 DB 마이그레이션이 불필요하고 팀 전원이 동일한 도구를 쓸 수 있지만, GCP 크레딧을 포기해야 합니다.

GPU 워크로드는 Q3에서 RunPod으로 외부화하기로 결정했으므로 클라우드 선택과 무관합니다. 결국 **DB가 이미 AWS에 있다는 점**과 **5명 팀에서 멀티클라우드 운영은 지속 불가능하다는 점**이 결정적이었습니다. Dev 환경은 별도로 운영하고, 이 클러스터는 **Prod 전용**으로 구성합니다.

> **결정: AWS 단일 클러스터, Prod 전용 (Dev 별도 운영)**

---

## 5. Q2: DB 위치 — K8S 내부 vs 외부?
...
## 6. Q3: GPU 처리 — K8S 내부 vs RunPod 외부?
...
따라서 Q1에서 단일 클라우드(AWS)로 통합하되, **GPU만 RunPod으로 외부 위임**합니다. AI Server(FastAPI)는 K8S 내부에서 오케스트레이터로 동작하며, vLLM에 HTTPS API 요청을 보내는 구조입니다. 비용 측면에서도 RunPod Spot은 월 ~$140으로 AWS g4dn.xlarge(월 ~$380)의 절반 이하입니다.
...
## 7. Q4: 노드 사이징 — 인스턴스 타입, 수량

Q1에서 결정한 **Prod 전용 클러스터**(Dev 별도 운영)에 대해, Prod 워크로드 합산은 CPU 3950m, RAM 10.0GB(HA 포함, [node-sizing.md](./node-sizing.md) 참조)입니다. 핵심 제약 조건은 **N-1 장애 허용**(노드 1대 다운 시에도 Prod 유지)과 **비용 vs 여유 자원의 균형**입니다. 또한 T 시리즈(Burstable)의 CPU 크레딧 고갈이 실측으로 확인되었고(BE 90.3%, FE 92.8%), 현재 실측 데이터가 Dev 저트래픽 기준이라 Prod 부하는 더 높을 수 있다는 점도 고려해야 합니다.
...
## 10. Q7: 리소스 산정 — Request/Limit 방법론
...
## 12. Q9: 네트워크 대역 — Pod CIDR, Service CIDR
...
## 16. Q13: NetworkPolicy — Pod 간 통신 제어
...
| Q | 주제 | 결정 | 한줄 근거 |
|---|------|------|----------|
| Q1 | 클러스터 전략 | AWS 단일, Prod 전용 (Dev 별도) | 멀티클라우드 운영 부담 해소, 소규모 팀 |
