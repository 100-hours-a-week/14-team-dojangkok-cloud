...
K8S 전환 시 다수의 서비스를 소수의 Worker 노드에 고집적 배치하므로, 각 서비스의 실제 리소스 사용량을 파악하는 것이 노드 사이징의 출발점이다.
...
위 한계를 전제로, 초기 서비스 오픈 시 트래픽 변동성을 안전하게 흡수하기 위해 **보수적으로** 산정한다. 실측 데이터와 서비스 특성(데이터 정합성, API 지연 허용치 등)을 종합적으로 고려했으며, 부하테스트 완료 후 전면 재조정할 예정이다.
...
섹션 3에서 산출한 Prod HA 워크로드(~3950m)와 HPA 스케일아웃 시(최대 ~4700m)를 수용할 클러스터 노드를 선정한다. T시리즈 CPU 크레딧 고갈이 실측으로 확인되었고(BE 90.3%, FE 92.8%), 실측 데이터가 Dev 저트래픽 기준이라 Prod 부하는 더 높을 수 있으므로, **비용 대비 충분한 HPA 확장 여유** 를 확보하는 것이 핵심이다.
...
Prod HA(~3950m) 기준과 HPA 풀스케일(~4700m) 시를 고려하여 네 가지 구성을 비교했다.
...
**t4g.large × 4** 가 xlarge × 2와 **동일 비용($196)** 이면서, 평시 HA 활용률 66%로 HPA 스케일아웃(FE+BE 각 1대 추가) 시에도 78%로 수용 가능하다. 노드 4대이면 **AZ 2:2 균등 배치** 가 가능하고, RAM 활용률 38%로 InnoDB Buffer Pool 증설이나 MongoDB 데이터 증가에도 충분하다.
...
- 둘째, DB HA 채택 시 Primary/Replica는 서로 다른 AZ에 배치해야 AZ 장애에 대비할 수 있다(섹션 10에서 논의 중).
- 셋째, DB HA 채택 시 Quorum 과반이 Primary 반대 AZ에 있어야 자동 선출이 보장된다(섹션 10에서 논의 중).
...
kubeadm은 EKS와 달리 CNI가 기본 내장되어 있지 않아 별도 설치가 필요하다. CNI는 Pod 네트워크의 기반으로, 이후 모든 네트워크 설계(대역, 캡슐화, NetworkPolicy)에 영향을 미친다. K8S는 기본적으로 모든 Pod이 같은 flat network에 존재하므로, V2에서 프라이빗 서브넷 분리 + 보안그룹으로 확보했던 네트워크 격리를 K8S에서도 유지하려면 **NetworkPolicy를 지원하는 CNI가 필수**다.
...
AWS VPC 환경에서 BGP 피어링이 불가하므로 None(Direct)은 제외된다. IPIP와 VXLAN은 모두 동작하지만, VXLAN이 BGP 메시를 필요로 하지 않고 향후 eBPF 데이터플레인 전환 시 호환성이 우수하다.

각 모드에는 **Always**와 **CrossSubnet** 옵션이 있다.
- **Always**: 모든 Pod 트래픽을 캡슐화한다.
- **CrossSubnet**: 같은 서브넷 내에서는 Direct routing, 다른 서브넷 간에만 캡슐화한다.

우리 환경(2 AZ, Worker 2:2, AZ별 서브넷 분리)에서는:
- 같은 AZ(같은 서브넷) 내 Pod 통신 → 캡슐화 불필요, Direct가 최적
- 다른 AZ(다른 서브넷) 간 Pod 통신 → 캡슐화 필요

따라서 **VXLAN CrossSubnet**을 선택한다. 같은 서브넷에서는 오버헤드 없이 Direct routing하고, 다른 서브넷 간에만 VXLAN 캡슐화(~50B, 실질 ~2-3% 오버헤드)를 적용한다.

### AWS 인프라 요구사항

VXLAN CrossSubnet 운용에 필요한 AWS 측 설정이다.
...
> **결정: Calico + VXLAN CrossSubnet — CNI+NetworkPolicy 단일 컴포넌트, 같은 AZ Direct routing + 다른 AZ VXLAN 캡슐화. AWS source/dest check 비활성화 필요**

CNI와 캡슐화 모드가 정해졌으니, kubeadm init 시 지정할 Pod/Service CIDR을 설계한다.

---

## 7. 네트워크 대역 — Pod CIDR, Service CIDR

섹션 6에서 Calico(VXLAN CrossSubnet)를 선택했다. kubeadm init 시 Calico가 사용할 Pod CIDR과 Service CIDR을 지정해야 하며, VPC 대역과 겹치면 라우팅 충돌이 발생한다. 신규 VPC 대역은 다음과 같다.
...
내부 네트워크(CNI + 대역 + kube-proxy)가 정해졌다. 다음은 private 서브넷에서 외부로 나가는 출구를 설계한다.

---

## 8. 외부 통신 — NAT, RunPod 호출 경로

섹션 6~7에서 클러스터 내부 네트워크를 설계했다. Worker 노드는 private 서브넷에 있으므로, 외부(RunPod API, ECR, 외부 API)와 통신하려면 NAT가 필요하다.

**NAT Gateway**(월 ~$32 + 데이터)는 AWS 관리형으로 자동 고가용성을 제공하지만 비용이 높다. **NAT Instance × 1**(월 ~$3.8)은 저비용이며 V2에서 운영 경험이 있지만, AZ 배치(섹션 5)에서 결정한 2:2 배치와 **논리적으로 상충**한다. NAT Instance가 2a에만 있을 때 2a AZ 장애가 발생하면, 2c에 살아남은 Worker(W3, W4)가 인터넷 출구를 잃어 RunPod API 호출·ECR 이미지 풀·HPA 스케일아웃이 모두 불가능해지므로, 2:2 배치로 확보한 고가용성이 무의미해진다.
...
| Q8 | CNI | Calico + VXLAN CrossSubnet | V2 보안그룹 → NetworkPolicy 계승, 같은 AZ Direct + 다른 AZ VXLAN |
...
**네트워크 보안**: Calico (VXLAN CrossSubnet) NetworkPolicy (Default Deny) + 공통 필수 정책 (kube-dns, Gateway→App, 외부 Egress)
