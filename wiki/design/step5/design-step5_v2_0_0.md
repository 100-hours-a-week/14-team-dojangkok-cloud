...
17. [NetworkPolicy — Pod 간 통신 제어](#17-networkpolicy--pod-간-통신-제어)
18. [RBAC/SA — K8S API 접근 제어](#18-rbacsa--k8s-api-접근-제어)

---
19. [설계 결정 요약](#19-설계-결정-요약)
20. [비용 비교](#20-비용-비교)
21. [장애 대응 & Failover](#21-장애-대응--failover)
22. [부록](#22-부록)
...
- **대응 방안**: 수백 대의 노드를 굴리는 엔터프라이즈 환경에서는 부담스럽지만, 도장콕의 초기 인프라 규모(노드 5대(CP 1 + Worker 4), 섹션 4에서 상세 산정)에서는 롤링 업데이트 방식으로 다운타임 없이 30분 내외로 작업이 가능하므로 팀의 현재 운영 리소스로 충분히 감당할 수 있다.
...
위 실측은 **Dev 저트래픽 환경**(실 사용자 없음, 내부 테스트)에서 수집된 것으로, Prod 부하와는 차이가 있을 수 있다. MongoDB와 ChromaDB는 V3에서 신규 추가되어 실측 데이터 자체가 없다.

이 한계를 인지한 상태에서, 다음 2단계로 보완한다.

1. **사전 검증**: K8S 전환 전 도커 컨테이너 기반 부하테스트를 통해 Prod 수준의 트래픽을 인가하고 Request/Limit을 1차 보정한다.
2. **운영 최적화**: Prod 전환 후 Prometheus/Grafana로 수집된 프로덕션 메트릭과 HPA 스케일링 트렌드를 분석하여 점진적으로 최적화한다.

### 초기 리소스 산정 (잠정)

위 한계를 전제로, 초기 서비스 오픈 시 트래픽 변동성을 안전하게 흡수하기 위해 **보수적으로** 산정한다. 실측 데이터와 서비스 특성(데이터 정합성, API 지연 허용치 등)을 종합적으로 고려했으며, 부하테스트 완료 후 재조정할 예정이다.

| 서비스 | 실측 근거 | CPU Req | CPU Lim | RAM Req | RAM Lim |
|--------|----------|---------|---------|---------|---------|
| FE | avg 0.037v, peak 1.86v, RSS ~400MB | 250m | 1500m | 768MB | 1.5GB |
| BE | avg 0.072v, peak 1.81v, JVM RSS ~900MB | 500m | 2000m | 1.5GB | 3GB |
| AI Server | avg ~0.04v, peak 0.23v, RSS 221MB | 200m | 500m | 512MB | 1GB |
| MySQL | avg 0.024v, peak 0.063v, BP 512MB→1GB | 200m | 500m | 1.5GB | 3GB |
| MongoDB (신규) | 실측 없음, 보수적 추정 | 150m | 500m | 768MB | 1.5GB |
...
| MySQL | 1 Primary + 1 Replica + Orchestrator | StatefulSet + Deployment |
| MongoDB | 1 Primary + 1 Secondary + 1 Arbiter | StatefulSet + Deployment |
| Redis | 1 (단일, HA 보류) | Deployment |
...
|----------|------|-------------|-------------|
| App | FE×2, BE×2, AI×1 | 1700m | 5.05GB |
| DB (단일) | MySQL, MongoDB, Redis | 450m | 2.52GB |
| Infra | RabbitMQ, ChromaDB | 300m | 1.26GB |
| **워크로드 소계** | | **2450m** | **8.83GB** |
| 공유 컴포넌트 (잠정) | Gateway Fabric, ArgoCD, Prometheus, Grafana, Alloy 등 | ~850m | ~1.7GB |
| **Prod 합계** | | **~3300m** | **~10.5GB** |
...
|----------|------|-------------|-------------|
| **App** | FE×2, BE×2, AI×1 | 1700m | 5.05GB |
| **DB Primary** | MySQL, MongoDB, Redis(단일) | 450m | 2.52GB |
| **DB Replica** | MySQL Replica, MongoDB Secondary | 350m | 2.27GB |
| **Failover/Quorum** | MySQL Orchestrator, MongoDB Arbiter | 150m | 0.32GB |
| **Infra** | RabbitMQ, ChromaDB | 300m | 1.26GB |
| **워크로드 소계** | | **2950m** | **11.42GB** |
| 공유 컴포넌트 (잠정) | Gateway Fabric, ArgoCD, Prometheus, Grafana, Alloy 등 | ~850m | ~1.7GB |
| **Prod 합계** | | **~3800m** | **~13.1GB** |

> Redis HA 적용 시(Sentinel): +250m / +0.45GB → ~4050m / ~13.6GB
...
|----------|---------|---------|----------|
| 기본 HA (FE×2, BE×2) | — | — | ~3800m |
| FE +1 (→3대) | +250m | +768MB | ~4050m |
| BE +1 (→3대) | +500m | +1.5GB | ~4300m |
| FE+BE 각 +1 | +750m | +2.3GB | ~4550m |
...
섹션 3에서 산출한 Prod HA 워크로드(~3800m)와 HPA 스케일아웃 시(최대 ~4550m)를 수용할 클러스터 노드를 선정한다. T시리즈 CPU 크레딧 고갈이 실측으로 확인되었고(BE 90.3%, FE 92.8%), 실측 데이터가 Dev 저트래픽 기준이라 Prod 부하는 더 높을 수 있으므로, **비용 대비 충분한 HPA 확장 여유** 를 확보하는 것이 핵심이다.
...
Prod HA(~3800m) 기준과 HPA 풀스케일(~4550m) 시를 고려하여 네 가지 구성을 비교했다.

| 옵션 | Alloc CPU | Alloc RAM | 월비용 | HA 활용률 | HPA 풀(FE+BE +1) |
|------|----------|----------|--------|----------|---------|
| t4g.large × 3 | 4.5v | 19.5GB | $147 | 84% | ✗ (101%) |
| **t4g.large × 4** | **6.0v** | **26.0GB** | **$196** | **63%** | **76%** |
| t4g.xlarge × 2 | 7.0v | 29.0GB | $196 | 54% | 65% |
| t4g.xlarge × 3 | 10.5v | 43.5GB | $294 | 36% | 43% |

**t4g.large × 4** 가 xlarge × 2와 **동일 비용($196)** 이면서, 평시 HA 활용률 63%로 HPA 스케일아웃(FE+BE 각 1대 추가) 시에도 76%로 수용 가능하다. 노드 4대이면 **AZ 2:2 균등 배치** 가 가능하고, RAM 활용률 50%로 InnoDB Buffer Pool(1GB)과 MongoDB WiredTiger 캐시에 여유 있게 배분했으며, 추가 증설 여지도 있다.

> **결정: CP t4g.medium × 1 + Worker t4g.large × 4 (2vCPU, 8GB) — Prod 전용. xlarge×2와 동일 비용이면서 HPA 확장 여유(76%) + AZ 균등 배치 가능**
...
| **HPA 풀스케일 시 76%** — FE+BE 동시 +1 시 여유 24% | 추가 확장 여지 제한 | HPA maxReplicas 제한(FE 3, BE 3). 상시 80%+ 시 W5 추가 |

---

## 5. AZ 배치 — 노드별 워크로드 배분

노드 사이징(섹션 4)에서 결정한 **CP 1대 + Worker 4대** 를 ap-northeast-2의 2개 AZ(2a, 2c)에 어떻게 배분할지가 핵심이다. 고려해야 할 제약은 세 가지다.
- 첫째, StatefulSet의 EBS는 AZ에 종속되므로 **같은 AZ에 Worker 2대 이상** 이 있어야 노드 장애 시 재스케줄링이 가능하다.
- 둘째, DB HA(§12)에서 Primary/Replica를 서로 다른 AZ에 배치하여 AZ 장애에 대비한다.
- 셋째, MongoDB ReplicaSet의 Quorum(Arbiter 포함)을 Primary 반대 AZ에 배치하여 AZ 장애 시 자동 선출을 보장한다(§12에서 확정).

1:3이나 3:1 배치는 한쪽 AZ에 Worker가 1대뿐이라 재스케줄링이 불가능하다. **2:2 균등 배치** (AZ 2a에 CP+W1+W2, AZ 2c에 W3+W4)만이 양쪽 AZ 모두에서 Worker 2대씩을 확보하여, 어느 AZ에서든 노드 1대 장애 시 같은 AZ의 나머지 노드로 재스케줄링할 수 있다. DB Primary/Replica를 AZ 분리 배치하여 AZ 장애 시 데이터를 보존하고, FE/BE는 Anti-Affinity로 AZ에 분산하여 어느 AZ가 다운되어도 서비스를 유지한다.

단일 Control Plane(2a) 장애 시, 클러스터 제어권(스케줄링, 배포)은 일시 상실되나 기존 Worker 노드(2a, 2c)에 배포된 Data Plane(FE, BE 파드 및 라우팅 룰)은 정상 작동하여 **사용자 서비스 중단은 발생하지 않는다** . CP 복구는 S3에 자동화된 etcd 스냅샷을 통해 수행한다(섹션 2 "kubeadm 선택 근거" 단점 1 참조).
...
| **Cilium** | eBPF 기반 고성능, L7 정책, Observability 내장 | 커널 5.10+ 요구, 에이전트당 ~300MB(4대 1.2GB), 러닝커브 |
...
> **결정: Calico + VXLAN — CNI+NetworkPolicy 단일 컴포넌트, 모든 Pod 트래픽 VXLAN 캡슐화. AWS source/dest check 비활성화 필요**
...
Service의 트래픽 라우팅을 담당하는 kube-proxy는 Kubeadm의 기본값인 **iptables**를 사용한다. 노드 5개(Worker 4개)와 Service 수십 개 이내의 초기 클러스터 규모에서는 iptables 룰 순차 탐색으로 인한 성능 병목(O(N) 탐색)이 거의 발생하지 않는다. 도입 초기의 복잡도를 낮추기 위해 커널 모듈 추가 작업이 필요한 IPVS(O(1) 해시 테이블)는 추후 대규모 트래픽 발생이나 서비스 개수가 폭증할 때 전환을 검토한다.
...
<br> **NAT Instance × 1**(월 ~$3.8)은 저비용이며 V2에서 운영 경험이 있지만, AZ 배치(섹션 5)에서 결정한 2:2 배치와 **논리적으로 상충** 한다. <br>NAT Instance가 2a에만 있을 때 2a AZ 장애가 발생하면, 2c에 살아남은 Worker(W3, W4)가 인터넷 출구를 잃어 RunPod API 호출·ECR 이미지 풀·HPA 스케일아웃이 모두 불가능해지므로, 2:2 배치로 확보한 고가용성이 무의미해진다.

이를 해결하기 위해 **NAT Instance × 2를 AZ별로 배치**하고, **라우팅 테이블을 AZ별로 분리**한다. 각 AZ의 private 서브넷이 자기 AZ의 NAT Instance만 바라보도록 구성하면, 한쪽 AZ가 다운되어도 반대쪽 AZ는 독립적인 인터넷 출구를 유지한다. 추가 비용은 월 ~$3.8(t4g.nano 1대)에 불과하다.
...
섹션 5에서 Worker 노드를 2a(W1, W2)와 2c(W3, W4)로 2:2 균등 배치했다. 파드가 우연히 단일 AZ 노드로만 몰려서(예를 들어 BE 파드 2개가 모두 W1, W2에만 스케줄링 됨) AZ 장애 시 서비스가 중단되는 것을 막기 위해 **Pod Anti-Affinity**를 설정한다.
...
**AWS EBS CSI Driver를 채택한다.** AWS 단일 클라우드로 통합한 이상 EBS 네이티브 연동이 가장 자연스럽고, OpenEBS/Longhorn의 분산 스토리지 오버헤드는 Worker 4대 규모에서 리소스 낭비다. 권한은 §2(kubeadm 단점 3)에서 언급한 대로 워커 노드의 IAM Instance Profile(AmazonEBSCSIDriverPolicy)로 부여한다.
...
AZ 2c: W3, W4 + EBS-B (MySQL Replica)

W1 장애 → W2로 재스케줄링 → EBS-A 재연결 ✓ (같은 AZ)
AZ 2a 전체 장애 → W3/W4로 재스케줄링 → EBS-A 접근 불가 ✗ (다른 AZ)
```

§5의 2:2 AZ 배치에서 각 AZ에 Worker 2대씩 있으므로, **같은 AZ 내 노드 장애는 재스케줄링으로 복구 가능**하다. 그러나 AZ 전체 장애 시 해당 AZ의 EBS에 의존하는 단일 Pod는 복구할 수 없다.
...
전부 단일 인스턴스, HA 없음. 장애 시 수동 복구가 필요하다.
...
[K8S Cluster — 새 VPC Private 서브넷]
  ...
  Gateway Fabric (DaemonSet)
       ...
       │ Private IP (같은 VPC)
       ▼
[EC2 — 새 VPC Private 서브넷]
  MySQL, MongoDB, Redis, RabbitMQ, ChromaDB
```
...
- **리소스 분리**: DB 전용 리소스가 보장되어 App과 CPU/메모리 경합이 없음.
...
V2에서는 모든 Stateful 워크로드가 단일 인스턴스로 운영되어, 장애 시 수동 복구가 필요했다. Phase 2에서 K8S로 이관할 때 **MySQL, MongoDB는 자동 Failover를 확보**하고, 나머지(Redis, RabbitMQ, ChromaDB)는 현재 규모와 복구 가능성을 고려하여 **단일 구성으로 시작**한다.

### 확정 결정

| 워크로드 | 구성 | Failover | AZ 배치 |
|----------|------|----------|---------|
| MySQL | P + R + Orchestrator | 자동 (Orchestrator) | P(2a), R+Orch(2c) |
| MongoDB | P + S + Arbiter | 자동 (내장 election) | P(2a), S+A(2c) |
| Redis | 단일 (HA 보류) | — | 단일 Pod |
| RabbitMQ | 단일 + PVC | — | 단일 Pod |
| ChromaDB | 단일 + PVC + 정기 백업 | — | 단일 Pod |

### MySQL — Primary + Replica + Orchestrator

MySQL은 자체 자동 failover 메커니즘이 없다. Group Replication(최소 3노드)은 현재 규모에서 리소스 과잉이고, Percona Operator(Galera + HAProxy)는 학습/운영 복잡도가 높다.

**채택 구성: Primary + Replica + semi-sync replication + Orchestrator**

- **Primary (AZ 2a)**: 읽기/쓰기 처리
- **Replica (AZ 2c)**: semi-sync로 Primary와 동기화, 읽기 분산 가능
- **Orchestrator (AZ 2c, Deployment)**: Primary 장애 감지 → Replica 자동 promote + Service Endpoint 전환

Orchestrator를 Replica와 같은 AZ(2c)에 배치하는 이유: Primary AZ(2a) 장애 시 Orchestrator가 살아남아 Replica를 자동 promote할 수 있다. 반대로 2c 장애 시 Primary는 독립 운영 가능하고(쓰기 지속), Orchestrator 복구까지 자동 failover만 불가할 뿐 서비스 중단은 없다.

| 컴포넌트 | CPU Req | RAM Req |
|----------|---------|---------|
| Primary | 200m | 1.5GB |
| Replica | 200m | 1.5GB |
| Orchestrator | 100m | 256MB |
| **합계** | **500m** | **3.26GB** |

### MongoDB — ReplicaSet (Primary + Secondary + Arbiter)

MongoDB ReplicaSet은 과반(majority) 투표로 Primary를 자동 선출한다. MySQL과 달리 외부 failover 도구가 불필요하다.

**채택 구성: Primary + Secondary + Arbiter (StatefulSet + Deployment)**

- **Primary (AZ 2a)**: 읽기/쓰기 처리
- **Secondary (AZ 2c)**: oplog 기반 실시간 복제, 읽기 분산 가능
- **Arbiter (AZ 2c, Deployment)**: 데이터 미저장, 투표만 참여 (50m/64MB)

Arbiter를 Secondary와 같은 AZ(2c)에 배치하여, Primary AZ(2a) 장애 시 2c의 S+A가 과반(2/3)을 확보 → Secondary 자동 승격.

| 컴포넌트 | CPU Req | RAM Req |
|----------|---------|---------|
| Primary | 150m | 768MB |
| Secondary | 150m | 768MB |
| Arbiter | 50m | 64MB |
| **합계** | **350m** | **1.60GB** |

### Redis — 단일 (HA 보류)

현재 Redis 역할은 세션 저장과 캐시다. 유실 시 재생성 가능하므로, 단일 Pod + PVC로 시작한다.

V3 채팅 기능에서 다중 BE Pod 간 WebSocket 메시지 분배를 위한 Pub/Sub 역할이 추가될 수 있다. BE 설계 확정 후 HA 수준을 결정한다.

- **HA 필요 시**: Sentinel (Master + Replica + Sentinel×3) → 추가 +250m / +448MB

### RabbitMQ — 단일 + PVC

AI 요청/응답 큐 용도로, 메시지 유실 시 클라이언트 재요청으로 복구 가능하다. Quorum Queue(3노드 Raft)는 리소스 3배(+200m/512MB × 2) + Erlang 클러스터 운영 복잡도가 높아 현재 규모에서 과잉이다.

PVC(Retain) + 정기 EBS 스냅샷으로 데이터를 보존한다.

### ChromaDB — 단일 + PVC + 정기 백업

ChromaDB는 자체 HA를 지원하지 않는다. 벡터 인덱스는 원본 문서에서 재구축 가능하므로, 단일 Pod + PVC(Retain) + CronJob EBS 스냅샷으로 복구 경로를 확보한다.

### 2-AZ Quorum 제약

2-AZ에 3멤버를 배치하면 한쪽 AZ에 과반(2/3)이 집중된다.

| 장애 AZ | MySQL | MongoDB |
|---------|-------|---------|
| **2a (P)** | Orch가 R promote ✓ | S+A 과반 → S 승격 ✓ |
| **2c (R/Orch, S/A)** | P 독립 운영, 자동 failover 불가 (수동 개입) | P만 생존, 과반 미달 → 읽기 전용 |

2c 장애 시 MySQL은 Primary가 쓰기를 계속하고, MongoDB는 읽기 전용으로 강등된다. 이는 2-AZ의 근본적 한계이며, 3-AZ면 해결되지만 Worker 4대에서 2+1+1 불균등 배치가 된다. 전체 AZ 장애 확률이 매우 낮고, 발생 시 수동 개입을 허용하는 것으로 감수한다.

### 리소스 영향

| 항목 | CPU | RAM |
|------|-----|-----|
| MySQL HA (P+R+Orch) | +300m | +1.76GB |
| MongoDB HA (P+S+A) | +200m | +0.83GB |
| **HA 추가 합계** | **+500m** | **+2.59GB** |

Prod alloc 여유 6000m 기준으로 충분히 수용 가능하다. §3 시나리오 B 참조.

> **결정: MySQL P+R+Orchestrator(semi-sync + 자동 failover), MongoDB ReplicaSet P+S+A(내장 election), Redis 단일(HA 보류 — BE 설계 확정 후), RabbitMQ/ChromaDB 단일+PVC. 2-AZ quorum 제약 감수.**
...
Ingress(섹션 14)에서 NGINX Gateway Fabric을 확정한 이상, **모든 외부 트래픽은 Gateway Fabric을 통해서만 진입** 하는 구조가 자연스럽습니다. FE/BE를 별도 NodePort로 직접 노출하면 라우팅 룰이 ALB와 Gateway에 이원화되고, Gateway Fabric의 관측성(access log, 메트릭) 이점이 사라집니다.
...
- **병목 대응**: 모든 외부 트래픽이 Gateway를 경유하므로, Gateway Pod가 단일 노드에만 있으면 SPOF가 된다. DaemonSet으로 모든 Worker(W1~W4)에 각 1개씩 배치하고, ALB Target Group이 4대의 동일 NodePort를 바라보게 한다. 노드 1대 다운 시 ALB health check가 감지하여 나머지 3대로 분산하며, 트래픽이 도착한 노드에서 로컬 Gateway Pod이 직접 처리하므로 노드 간 추가 hop이 없다. §5의 2:2 AZ 배치와 맞물려 AZ 장애 시에도 반대쪽 AZ의 Gateway Fabric이 트래픽을 계속 처리한다.
...
## 18. RBAC/SA — K8S API 접근 제어

§17에서 Pod 간 네트워크 통신을 제어했다. 하지만 보안에는 또 다른 축이 있다 — **누가 K8S 리소스를 조회·생성·삭제할 수 있는가**. V2에서는 IAM 정책과 SSH 키로 AWS 리소스 접근을 제한했다. K8S에서도 동일한 수준의 접근 제어가 필요하며, 이를 RBAC(Role-Based Access Control)와 ServiceAccount로 구현한다.

```
§17 NetworkPolicy: "Pod끼리 아무나 통신하면 안 된다" (네트워크 격리)
   ↓ "네트워크는 통제했다. 그런데 K8S API 접근은?"
§18 RBAC/SA: "아무나 K8S 리소스를 건드리면 안 된다" (API 접근 제어)
```
...
구체적인 SA 목록(EBS CSI Driver, ArgoCD, Prometheus, Gateway Fabric 등)과 각각의 Role/ClusterRole 바인딩은 **구현 단계에서 정의**한다. 본 설계에서는 아래 원칙을 확정한다.

1. **default SA 사용 금지** — 모든 워크로드에 전용 SA 생성
2. **automountServiceAccountToken: false 기본** — API 접근 불필요 Pod에 토큰 미마운트
3. **Namespace 단위 Role 우선** — ClusterRole은 Namespace 밖 리소스 접근 시에만 사용
4. **Secret 접근 최소화** — Secret을 읽을 수 있는 SA를 제한하여 민감 데이터 노출 방지

> **결정: 워크로드별 전용 SA + 최소 권한 RBAC — V2 IAM/SSH 접근 제어를 K8S에서 동등하게 계승. 구체 SA/Role 목록은 구현 단계에서 정의**
...
| Q4 | 노드 사이징 | t4g.large × 4 + CP t4g.medium × 1 | 실측 기반, HPA 확장 여유(76%), 2:2 AZ 균등 |
...
| Q13 | 외부 통신 | NAT Instance × 2 (t4g.nano, AZ별) | 2:2 AZ 배치와 일치, 월 +$3.8 |
