...
V2에서도 ALB TLS 종료로 운영한 전례가 있고, 내부 트래픽은 private 서브넷 안이라 평문이어도 실질적 위험이 낮습니다. ACM 인증서가 무료이므로 cert-manager 없이 운영 가능하며, kubeadm 내부 인증서(API Server, etcd 등)는 kubeadm init 시 자동 생성되며, **기본 1년 만료**입니다. `kubeadm certs renew`로 수동 갱신하거나, `kubeadm upgrade` 실행 시 자동 갱신됩니다. 만료 전 Prometheus 알림 설정이 필수입니다.
