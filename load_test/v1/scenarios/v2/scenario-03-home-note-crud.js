 ...
 *
 * 흐름: (인증) → 집 노트 생성 → 제목 수정 → 이미지 업로드 풀 사이클(0~5장 랜덤)
 *       → 체크리스트 조회 → 체크리스트 항목 토글(여러 개) → 집 노트 목록 확인 → 집 노트 삭제
 *
 * v1 대비 변경 사항:
 *   - 이미지 업로드 풀 사이클 추가 (Presigned URL → S3 업로드 → 완료 처리 → 첨부)
 *   - property-1~5.jpg 중 0~5장 랜덤 선택하여 업로드 (0장이면 스킵)
 *
...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{name:POST /home-notes/{id}/files/presigned-urls}': ['p(95)<2000'],
...
/**
 * 0~5장 랜덤 선택. 0장이면 빈 배열 반환.
 */
function pickRandomImages() {
  const count = Math.floor(Math.random() * (PROPERTY_IMAGES.length + 1)); // 0~5
  if (count === 0) return [];
...
export default function (data) {
  ...
  try {
    ...
    // Step 2: 제목 수정
    updateHomeNoteTitle(token, homeNoteId, `${title} (수정됨)`);
    sleep(0.5);

    // Step 3: 이미지 업로드 풀 사이클 (0~5장 랜덤)
    const selectedImages = pickRandomImages();
    if (selectedImages.length > 0) {
      const fileItems = selectedImages.map((s) => s.fileItem);
      const presignedData = getFilePresignedUrls(token, homeNoteId, fileItems);
      ...
      if (presignedData && presignedData.file_items && presignedData.file_items.length > 0) {
        ...
        for (let i = 0; i < presignedData.file_items.length; i++) {
          const uploaded = uploadToS3(
            presignedData.file_items[i].presigned_url,
            'image/jpeg',
            selectedImages[i].fileData
          );
          if (uploaded) {
            fileAssetIds.push(presignedData.file_items[i].file_asset_id);
          }
        }
        sleep(0.3);
        ...
        if (fileAssetIds.length > 0) {
          ...
          // 집 노트에 이미지 첨부
          attachFilesToHomeNote(token, homeNoteId, fileAssetIds);
          sleep(0.5);
        }
      }
    }
    ...
    // Step 7: 집 노트 삭제 (테스트 데이터 정리 겸 삭제 부하 측정)
